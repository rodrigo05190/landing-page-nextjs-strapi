...
const WhatsApp: React.FC<IdProduct> = ({ id_produto }, gtag) => {
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      const response = await axios.post(
        `https://trisul.controleid360.com.br/api/lead/whatsapp`,
        {
          ...values,
          ...config,
          ...session,
      ...
      if (product?.gtag) {
        if(window.location.pathname == '/studios/isla-vila-madalena') {
          ...
          (window as any).gtag('event', 'conversion', {
            send_to: 'AW-11093319275/JLG_CIrz2v4YEOu82qkp',
            send_id: 'AW-11093319275',
          });

        } else if(window.location.pathname == '/studios/horizon-studios') {
          (window as any).gtag('event', 'conversion', {
            send_to: 'AW-11093319275/ibdwCKypz_8YEOu82qkp',
            send_id: 'AW-11093319275',
          });
        } else {
          (window as any).gtag('event', 'conversion', {
